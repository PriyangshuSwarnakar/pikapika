...
Download `Pikapika_Setup_v1.2.exe` from the [Releases](../../releases) page and run it. No Python installation required.
...
Then follow the steps in `BUILD_INSTRUCTIONS.txt` to produce the `.exe` using PyInstaller and the included `pikapika.iss` Inno Setup script.
